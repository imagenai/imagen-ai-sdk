...
from imagen_sdk import (
    ImagenClient, ImagenError, AuthenticationError, ProjectError, UploadError, DownloadError,
    Profile, EditOptions, PhotographyType, StatusDetails, UploadResult, UploadSummary
)
    ...
    async def test_make_request_success_200(self, client, mock_helpers):
        ...
        with patch.object(client, '_get_session') as mock_session:
            ...
            mock_session.return_value.request.assert_called_once_with(
                'GET',
                'https://api-beta.imagen-ai.com/v1/test'
            )
...
class TestImagenClientDownloadFiles:
    ...
    async def test_download_files_success(self, client, mock_helpers):
        ...
        # Mock successful file downloads by patching the download_single_file inner function
        successful_paths = ["test_downloads/file1.jpg", "test_downloads/file2.jpg"]

        with patch.object(client, 'download_files') as mock_download:
            # Instead of mocking the entire method, let's test the actual implementation
            # by mocking the HTTP and file operations properly
            pass
    ...
    async def test_download_files_all_downloads_fail(self, client):
        ...
        download_links = ["https://example.com/file1.jpg", "https://example.com/file2.jpg"]
...
class TestImagenClientDownloads:
    ...
    async def test_get_download_links_success(self, client, mock_request_factory):
        files_data = [
            {"file_name": "image1.jpg", "download_link": "https://example.com/download1.jpg"},
            {"file_name": "image2.jpg", "download_link": "https://example.com/download2.jpg"}
        ]
        ...
        with mock_request_factory(response_data):
            ...
            client._make_request.assert_called_once_with(
                'GET',
                '/projects/test-uuid/edit/get_temporary_download_links'
            )

    @pytest.mark.asyncio
    async def test_get_export_links_success(self, client, mock_request_factory):
        files_data = [{"file_name": "export1.jpg", "download_link": "https://example.com/export1.jpg"}]
        response_data = self._create_download_response(files_data)
        ...
        with mock_request_factory(response_data):
            ...
            client._make_request.assert_called_once_with(
                'GET',
                '/projects/test-uuid/export/get_temporary_download_links'
            )
...
class TestImagenClientFileOperations:
    ...
    async def test_upload_to_s3_failure(self, mock_helpers):
        ...
        mock_response = Mock()
        mock_response.raise_for_status = Mock(
            side_effect=httpx.HTTPStatusError("Upload failed", request=Mock(), response=Mock())
        )
        mock_client = mock_helpers.create_async_http_client_mock(mock_response)
            ...
            with mock_request_factory(presigned_response):
                with patch.object(client, '_calculate_md5', return_value="test-md5-hash"):
                    with patch.object(client, '_upload_to_s3'):
                        ...
                        expected_payload = {
                            "files_list": [
                                {"file_name": "img1.jpg", "md5": "test-md5-hash"}
                            ]
                        }
                        client._make_request.assert_called_once_with(
                            'POST',
...
class TestImagenClientIntegration:
    ...
    async def test_full_workflow_simulation(self, client):
        ...
        upload_response = {
            "data": {
                "files_list": [
                    {"file_name": "test.jpg", "upload_link": "https://s3.com/upload"}
                ]
            }
        }
        ...
        download_response = {
            "data": {
                "files_list": [
                    {"file_name": "test_edited.jpg", "download_link": "https://download.com/test.jpg"}
                ]
            }
        }
